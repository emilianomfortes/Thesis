...
PROGRAM ISING
  ...
  REAL(8) :: GG,alpha_x,alpha_y,alpha_z,a(4),mu,ele,alpha2_x,alpha2_y,alpha2_z,tau,hr
  ...
  REAL(8), ALLOCATABLE, SAVE :: HH11(:,:),HH011(:,:),HH12(:,:),Tr(:,:),htr(:),Tra(:,:),shaper(:),ipr(:)
...
!  OPEN(3,FILE = 'Tr.out',status='unknown')
!  OPEN(25,FILE = 'hamiltsector.out',status='unknown')
...
!  open(15,file = 'statesparity', status = 'unknown')
...
!  OPEN(33,FILE = 'HHcompleto.out',status ='unknown')
  ...
  ALLOCATE(HH(dim,dim),energy(dim),vec1(dim),width(bins),HH1(dim,dim),vec(dim),histo(bins),HH0(dim,dim),Tr(dim,dim),Tra(dim,dim))
  ALLOCATE(work(3*dim**2),IWORK(6*dim),state(dim),flag(2**sites),ener_vec(dim),HHparity(dim,dim),statepp(dim),statepn(dim))
  ALLOCATE(HH0parity(dim,dim),energy0(dim),HH01(dim,dim),ener0_vec(dim),htr(sites))
...
!  htr(:) = htr(:) - 1
  ...
  Tra = 0.d0
   do i=1,dim
    if(btest(state(i),j).eqv..false.)Tra(i,i)=Tra(i,i)-0.5
    if(btest(state(i),j).eqv..true.)Tra(i,i)=Tra(i,i)+0.5
   enddo
  Tr(:,:) = Tr(:,:) + htr(j+1) * Tra(:,:)
  enddo
  do i=1,dim
...
!  neig = 1
!  CALL ising_chain_FN_p(GG,alpha_x,alpha_y,alpha_z,sites,dim_parity,boundary_con,neig,HH0,state_parity,flag_parity)
!  neig = 2
!  CALL ising_chain_FN_p(GG,alpha2_x,alpha2_y,alpha2_z,sites,dim_parity,boundary_con,neig,HH1,state_parity,flag)
!  HH = HH0 + mu*HH1
...
!  do i=1,dim_parity
!   do j=1,dim_parity
!    write(*,*)i,j,DOT_PRODUCT(HHparity(:,i),HHparity(:,j))
!   end do
!  end do

  ALLOCATE(ipr(dim),shaper(2))
  shaper = shape(HH(1,:))
  k = shaper(1)
  shaper = shape(HH(:,1))
  l = shaper(1)
...
DEALLOCATE(HH,energy,work,IWORK,vec1,width,histo,vec,ener_parity,ener0_parity,ener_vec,HH1,op,ipr,shaper)
